...
export default function Widgets({newsResults, randomUsersResults}) {
    const [articleNum, setArticleNum] = useState(3);  /* constante contenant le nombre de "news" a afficher avant de cliquer sur "show more" */
    const [randomUserNum, setRandomUserNum] = useState(3)
    return (
         /* création de la barre de recherche du widget */
        <div className="xl:w-[600px] hidden lg:inline ml-8 space-y-5">
            ...
            <h4 className="font-bold text-xl px-4"> What's happening </h4>             
                {newsResults.slice(0, articleNum).map((article)=>(
                    <News Key={article.title} article={article} /> /* widget + show more  */ 
                ))} 
            ...
            <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">   
                ...
                {randomUsersResults.slice(0,randomUserNum).map((randomUser) =>(
                    <div key={randomUser.login.username} className=" flex items-center px-4 py-2 cursor-pointer hover:bg-gray-100 ">
                        ...
                        <button className="ml-auto bg-black text-white rounded-full text-sm px-3.5 py-1.5 font-bold hover:bg-gray-700 ">Follow</button>
                    </div>
                ))}
